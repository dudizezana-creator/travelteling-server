...
function buildRevealPayload(room) {
  ...
  const answers = room.answersForCurrentQuestion.map((a) => {
    ...
    return {
      ...
      text: a.text,
      heartsFrom: a.heartsFrom || [],   // list of socket IDs who hearted this answer
    };
  });
  return {
...
io.on('connection', (socket) => {
  ...
  // ── create_room ────────────────────────────────────────────────────────────
  socket.on('create_room', ({ hostName, categories, maxQuestions, lang = 'en' }) => {
    if (!hostName) {
    ...
    rooms[code] = {
      ...
      state: STATE.LOBBY,
      answersForCurrentQuestion: [],   // [{ socketId, text, heartsFrom: [socketId,...] }]
